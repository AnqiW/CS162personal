...
int main(unused int argc, unused char *argv[]) {
  ...
  while (fgets(line, 4096, stdin)) {
    ...
    if (fundex >= 0) {
      ...
      // Parse tokens in to target buffer
      ...
      if (strchr(tokens_get_token(tokens, 0), '/')){
        ...
        if (pid == 0){
          ...
          //check whether there is redirection in the argument
          ...
          if (access(fin_path, F_OK) == 0){ 
            ...
            if (pid == 0){
              if (execv(fin_path, target) < 0) {
                printf("Got argument as an executable. execv failed");
              }
